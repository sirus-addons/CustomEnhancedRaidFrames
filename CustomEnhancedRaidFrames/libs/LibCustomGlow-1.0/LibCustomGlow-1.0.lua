...
local ADDON_NAME = ...
local textureList = {
	["empty"] = string.format([[Interface\AddOns\%s\Libs\LibCustomGlow-1.0\AM_29]], ADDON_NAME),
	["white"] = [[Interface\BUTTONS\WHITE8X8]],
	["shine"] = string.format([[Interface\AddOns\%s\Libs\LibCustomGlow-1.0\Artifacts]], ADDON_NAME),
}
...
local function InitAlphaAnimation(self)
	self.target = self.target or self:GetRegionParent()
	self.fromAlpha = self.fromAlpha or 1
	self.toAlpha = self.toAlpha or 0

	self.frameAlpha = self.target:GetAlpha()
	self.alphaFactor = self.fromAlpha - self.toAlpha
end
...
local function CreateAlphaAnim(group, target, order, duration, fromAlpha, toAlpha, delay, appear, onPlay, onFinished)
	...
	alpha.fromAlpha = fromAlpha
	alpha.toAlpha = toAlpha
...
local function configureButtonGlow(f, alpha)
	...
	-- CreateAlphaAnim(group, target,			order, duration, fromAlpha, toAlpha, delay, appear, onPlay, onFinished)
	-- CreateScaleAnim(group, target,			order, duration, x, y, delay, smoothing, onPlay)
	...
	f.animOut:SetScript("OnPlay", function(self)
		self.isPlaying = true
	end)
	f.animOut:SetScript("OnStop", function(self)
