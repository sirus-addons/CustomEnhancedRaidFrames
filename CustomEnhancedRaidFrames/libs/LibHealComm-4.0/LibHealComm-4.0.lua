...
local strbyte, strchar, strformat, strgsub, strlen, strmatch, strsplit = string.byte, string.char, string.format, string.gsub, string.len, string.match, string.split
...
if( not HealComm.compressGUID ) then
	...
	local function unescape(str)
		str = strgsub(str, "\255", "\000")
		str = strgsub(str, "\254\250", "\058")
		str = strgsub(str, "\254\251", "\064")
		str = strgsub(str, "\254\252", "\044")
		str = strgsub(str, "\254\253", "\255")
		return strgsub(str, "\254\249", "\254")
	end

	HealComm.fixedCompress = true
	HealComm.compressGUID = setmetatable({}, {
		__index = function(tbl, guid)
			local cguid = strmatch(guid, "0x(.*)")
			local str = strgsub(cguid, "(%x%x)", guidCompressHelper)
	...
	HealComm.decompressGUID = setmetatable({}, {
		__index = function(tbl, str)
			...
			local a, b, c, d, e, f, g, h = strbyte(usc, 1, 8)
			...
			local guid = strformat(dfmt, a, b, c, d, e, f, g, h)
...
if( playerClass == "DRUID" ) then
	LoadClassData = function()
		...
		-- Set data
		-- T5 Resto 2 piece, +6 seconds to Regrowth
		itemSetsData["Nordrassil"] = {30216, 30217, 30219, 30220, 30221}
		-- T7 Resto +5% more healing to Nourish per hot
	...
	--	itemSetsData["T9 Resto"] = {48102, 48129, 48130, 48131, 48132, 48153, 48154, 48155, 48156, 48157, 48133, 48134, 48135, 48136, 48137, 48142, 48141, 48140, 48139, 48138, 48152, 48151, 48150, 48149, 48148, 48143, 48144, 48145, 48146, 48147}
		-- T10 Resto 2 piece, 30% less healing lost on WG
	...
	--	itemSetsData["T9 Holy"] = {48595, 48596, 48597, 48598, 48599, 48564, 48566, 48568, 48572, 48574, 48593, 48591, 48592, 48590, 48594, 48588, 48586, 48587, 48585, 48589, 48576, 48578, 48577, 48579, 48575, 48583, 48581, 48582, 48580, 48584}
		...
		GetHealTargets = function(bitType, guid, healAmount, spellName, hasVariableTicks)
			if( activeBeaconGUID and activeBeaconGUID ~= guid and guidToUnit[activeBeaconGUID] and UnitIsVisible(guidToUnit[activeBeaconGUID]) ) then
				return strformat("%s,%s", compressGUID[guid], compressGUID[activeBeaconGUID]), healAmount
			elseif( hasVariableTicks ) then
				healAmount = tconcat(healAmount, "@")
			end
		...
		CalculateHealing = function(guid, spellName, spellRank)
			...
			-- T10 Holy +35% healing while Divine Illumination is active
	...
	LoadClassData = function()
		...
		GetHealTargets = function(bitType, guid, healAmount, spellName, hasVariableTicks)
			if( spellName == BindingHeal ) then
				return strformat("%s,%s", compressGUID[guid], compressGUID[playerGUID]), healAmount
	...
	LoadClassData = function()
		...
		GetHealTargets = function(bitType, guid, healAmount, spellName, hasVariableTicks)
			...
			if( glyphCache[55440] and guid ~= playerGUID and spellName == HealingWave ) then
				return strformat("%s,%d,%s,%d", compressGUID[guid], healAmount, compressGUID[playerGUID], healAmount *  0.20), -1
			elseif( hasVariableTicks ) then
				healAmount = tconcat(healAmount, "@")
			end
...
local function parseChannelHeal(casterGUID, spellID, amount, totalTicks, ...)
	...
	local ticksLeft = ceil(pending.duration / pending.tickInterval)

	loadHealList(pending, amount, 1, 0, ticksLeft, ...)
...
local function parseHotHeal(casterGUID, wasUpdated, spellID, tickAmount, totalTicks, tickInterval, ...)
	...
	if( not totalTicks or not tickAmount or not spellName or select("#", ...) == 0 ) then return end
...
local function parseHealEnd(casterGUID, pending, checkField, spellID, interrupted, ...)
	...
	if( not pending ) then
		if checkField == "id" then
			pending = pendingHeals[casterGUID][spellID]
		else
			...
			if( data.timeout ) then
				...
				if( data.timeout <= 0 ) then
					...
					elseif( data.type == "heal" ) then
						...
						if( bitType ) then
							local targets, amount = GetHealTargets(bitType, data[1], hasVariableTicks and amount or max(amount, 0), data.spellName, data, hasVariableTicks)
							parseHotHeal(playerGUID, false, data.spellID, amount, totalTicks, tickInterval, strsplit(",", targets))

							if( not hasVariableTicks ) then
								sendMessage(strformat("H:%d:%d:%d::%d:%s", totalTicks, data.spellID, amount, tickInterval, targets))
							else
								sendMessage(strformat("VH::%d:%s::%d:%s", data.spellID, amount, totalTicks, targets))
							end
						end
...
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(timestamp, eventType, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
	...
	elseif( ( eventType == "SPELL_AURA_APPLIED" or eventType == "SPELL_AURA_REFRESH" or eventType == "SPELL_AURA_APPLIED_DOSE" ) and bitband(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == COMBATLOG_OBJECT_AFFILIATION_MINE ) then
		...
		if( hotData[spellName] ) then
			...
			if( bitType ) then
				local targets, amount = GetHealTargets(bitType, destGUID, hasVariableTicks and amount or max(amount, 0), spellName, hasVariableTicks)
				...
				if( bombAmount ) then
					...
					sendMessage(strformat("B:%d:%d:%d:%s:%d::%d:%s", totalTicks, spellID, bombAmount, bombTargets, amount, tickInterval, targets))
				elseif( hasVariableTicks ) then
					sendMessage(strformat("VH::%d:%s::%d:%s", spellID, amount, totalTicks, targets))
				-- Normal hot, nothing fancy
				else
					sendMessage(strformat("H:%d:%d:%d::%d:%s", totalTicks, spellID, amount, tickInterval, targets))
				end
			end
		end
			...
			if( amount ) then
				...
				if( bombPending and bombPending.bitType ) then
					...
					if( bombAmount ) then
						...
						sendMessage(strformat("UB:%s:%d:%d:%s:%d:%d:%s", pending.totalTicks, spellID, bombAmount, compressGUID[destGUID], amount, pending.tickInterval, compressGUID[destGUID]))
						return
					end
				end
				...
				if( pending.hasVariableTicks ) then
					sendMessage(strformat("VU::%d:%s:%d:%s", spellID, amount, pending.totalTicks, compressGUID[destGUID]))
				else
					sendMessage(strformat("U:%s:%d:%d:%d:%s", spellID, amount, pending.totalTicks, pending.tickInterval, compressGUID[destGUID]))
				end
			end
		end
	...
	elseif( eventType == "SPELL_AURA_REMOVED" ) then
		...
		if( hotData[spellName] and bitband(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == COMBATLOG_OBJECT_AFFILIATION_MINE ) then
			parseHealEnd(sourceGUID, nil, "id", spellID, false, compressGUID[destGUID])
			sendMessage(strformat("HS::%d::%s", spellID, compressGUID[destGUID]))
		end
	end
end
...
function HealComm:UNIT_SPELLCAST_SENT(unit, spellName, spellRank, targetName)
	...
	castTarget = strgsub(targetName, "(.-)%-(.*)$", "%1")
...
function HealComm:UNIT_SPELLCAST_START(unit, spellName, spellRank, id)
	...
	if( type == DIRECT_HEALS ) then
		...
		sendMessage(strformat("D::%d:%d:%s", self.spellToID[nameID] or 0, amount or "", targets))
	elseif( type == CHANNEL_HEALS ) then
		parseChannelHeal(playerGUID, self.spellToID[nameID], amount, localTicks, strsplit(",", targets))
		sendMessage(strformat("C::%d:%d:%s:%s", self.spellToID[nameID] or 0, amount, ticks, targets))
	end
end
...
function HealComm:UNIT_SPELLCAST_SUCCEEDED(unit, spellName, spellRank, id)
	...
	sendMessage(strformat("S::%d:0", self.spellToID[nameID] or 0))
end
...
function HealComm:UNIT_SPELLCAST_STOP(unit, spellName, spellRank, id)
	...
	sendMessage(strformat("S::%d:1", self.spellToID[nameID] or 0))
end
...
function HealComm:UNIT_PET(unit)
	...
	if petGUID then
		...
		guidToGroup[petGUID] = guidToGroup[UnitGUID(unit)]
